...
```
settings.json Hooks
  ├─ SessionStart      → node inject.js --session-start
...
```
wiz/
├─ index.js             记忆数据库管理 (69 函数, 741 行)
├─ inject.js            上下文注入引擎 (59 函数, 959 行)
├─ extract_worker.js    后台提取 Worker (19 函数, 340 行)
├─ consolidate.js       会话结束处理器 (13 函数, 357 行)
├─ daemon.py            MCP 服务器 (1208 行, 18 个工具)
├─ graph.js             知识图谱引擎 (22 函数, 524 行)
├─ event_log.js         事件日志系统 (16 函数, 169 行)
