...
**V2.1 核心亮点：零弹窗架构升级** — C# launcher + wscript.exe 静默启动 + `ws.Run(SW_HIDE)` 全覆盖，consolidate 长任务也不再弹窗。

```
V1.x: node inject.js → cmd 窗口一闪而过  ❌
V2.0: launcher.exe → 短任务OK，长任务(consolidate)仍弹窗  ⚠️
V2.1: wscript.exe + ws.Run(SW_HIDE) → 零窗口  ✅
```
...
Wiz V2.1 对进程创建做了双层防御：

| 改动 | 说明 |
|------|------|
| `launcher.cs` | C# 原生 `/target:winexe` GUI 应用，本身不申请控制台 |
| VBS `ws.Run` + SW_HIDE | 弃用 `ws.Exec`（自带控制台），使用 `ws.Run path, 0, True` 完全静默 |
| wscript.exe 中介 | wscript 是纯 GUI 进程，CC 不经过 cmd.exe /c |
| 无中文路径 | VBS 部署到 `C:\Users\Public\`，路径用正斜杠，避开编码转义问题 |
...
| 改动 | 文件 | 说明 |
|------|------|------|
| VBS `ws.Exec` → `ws.Run` | `run_*.vbs` | `ws.Exec` 自带控制台窗口，改 `ws.Run` + SW_HIDE(0) 完全静默 |
| 路径正斜杠化 | settings.json + VBS | `\Users\Public\run.vbs` 中的 `\r` 被 CC 当作转义处理，改 `C:/Users/Public/` |
| 部署到无中文路径 | settings.json | VBS 脚本部署到 `C:\Users\Public\`，避开中文编码问题 |
| 扩大 consolidate 读取 | `consolidate.js` | transcript 窗口 100→300 行，有效条目 30→100，避免长会话尾部被截断 |
...
> **为什么不用直调 launcher.exe？** CC 的 `async: true` hook 通过 `cmd.exe /c` 启动，cmd.exe 是控制台程序，短任务一闪而过不易察觉，但 consolidate.js 长任务（~10s API）的弹窗明显。走 `wscript.exe` + `ws.Run(SW_HIDE)` 彻底绕过 cmd.exe，实现全零窗口。
  "hooks": {
    "SessionStart": [
      {
        "matcher": "",
...
> **如果不想编译 C#**，可以直接调 `node inject.js`，会有一个 cmd 窗口瞬间闪过后消失，不影响功能。
