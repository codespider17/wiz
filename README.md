...
<p align="center">
  <img src="https://img.shields.io/badge/version-4.0.0-blue" alt="V4.0.0">
...
V4.0 使用 `CLAUDE_CODE_SESSION_ID` 环境变量精确识别当前会话，彻底解决跨会话读错文件的问题。

| 改动 | 说明 |
|------|------|
| 会话 ID 优先 | 优先使用 `CLAUDE_CODE_SESSION_ID` 环境变量定位 transcript 文件 |
| 智能降级 | 若环境变量不可用，自动降级到 mtime 排序检测 |
| 竞态防护 | 新会话文件 2 秒内的修改被忽略，避免读取正在写入的文件 |

### 增强记忆提取

Worker 提取能力大幅提升，捕获更多对话细节：

| 参数 | V3.x | V4.0 | 说明 |
|------|------|------|------|
| `MIN_NEW_LINES` | 25 | 10 | 降低触发阈值，更频繁提取 |
| `maxChars` | 30,000 | 80,000 | 单次提取上下文增加 167% |
| 消息截取长度 | 800 | 1,500 | 保留更多对话细节 |
| 消息数量上限 | 100 | 300 | 处理更长的对话历史 |
| 截断策略 | 头部 | 尾部 | 保留最新话题（通常更重要） |

### 优化摘要生成

Episode 摘要格式升级为结构化中文输出：

```
V3.x: "Summarize this Claude Code session in 2-3 sentences..."
V4.0: "用中文总结这次 Claude Code 会话。要求：
       1. 列出讨论过的**所有**话题（不遗漏）
       2. 每个话题一句话概括
...
`consolidate.js` 现在自动将 AI 生成的会话摘要写入 `wiz_last_session.md`：

- 路径：`~/.claude/projects/C--Users-----/memory/wiz_last_session.md`
- 格式：标准 memory frontmatter + AI 摘要
- 时机：每次 SessionEnd 触发时自动更新
- 用途：新会话启动时可直接读取上一会话的完整摘要

### API 调用优化

`extract_worker.js` 的 API 调用简化：

```javascript
// V3.x: 包含 system prompt
api.callFast([{ role: 'system', content: HERMES_PROMPT }, { role: 'user', content: prompt }])

// V4.0: 仅 user prompt（HERMES_PROMPT 已嵌入 user content）
api.callFast([{ role: 'user', content: prompt }])
```
...
| 语义记忆 | 277+ | 360+ | 400+ |
| 技能索引 | 91 | 91 | 92 |
| Raw Episodic | 无 | 全量保存 | 全量保存 |
| 分层系统 | 无 | hot/warm/cold/frozen | hot/warm/cold/frozen |
| 跨会话连续 | 有 bug | 完全修复 | 精确检测 |
| 会话检测 | 无 | 基础 | CLAUDE_CODE_SESSION_ID |
...
run_*.vbs (ws.Run SW_HIDE, 零窗口)
  ...
  │     ├─ Phase 2: flash AI 合并调用 (1次API=技能+记忆)
  │     │     ├─ 技能+记忆合并推荐 (关键词预筛→AI精选)
  │     │     └─ 任务分解 (仅短任务触发, 条件调用)
...
**核心升级：精确会话检测 + 增强记忆提取 + 优化摘要**

| 改动 | 文件 | 说明 |
|------|------|------|
| 精确会话检测 | `consolidate.js` | 使用 `CLAUDE_CODE_SESSION_ID` 环境变量定位 transcript |
| 竞态防护 | `consolidate.js` | 新会话文件 2 秒内修改被忽略 |
| 降低提取阈值 | `extract_worker.js` | `MIN_NEW_LINES` 从 25 降至 10 |
| 增加上下文量 | `extract_worker.js` | `maxChars` 从 30K 增至 80K |
| 延长截取长度 | `extract_worker.js` | 消息截取从 800 增至 1500 字符 |
| 优化截断策略 | `extract_worker.js` | 截断保留尾部（最新话题优先） |
| 结构化摘要 | `consolidate.js` | 中文格式 + 话题列表 + 最后消息 |
| 自动保存摘要 | `consolidate.js` | 自动更新 `wiz_last_session.md` |
| 简化 API 调用 | `extract_worker.js` | 移除冗余 system prompt |

### V3.2.1 (2026-05-24)

**修复：consolidate.js 崩溃 + MCP 服务器注册**

| 改动 | 说明 |
|------|------|
| 修复 `consolidate.js` | 内联 transcript 辅助函数，修复 SessionEnd 崩溃 |
| 注册 MCP 服务器 | 创建 `.mcp.json`，注册 ctxproxy |
| 重命名日志前缀 | `[overmind]` → `[wiz]` |
...
| 改动 | 说明 |
|------|------|
| 删除 `lib/api.js` | 未使用的独立 API 实现，已被 `api_config.js` 替代 |
| 删除 `lib/feedback.js`、`lib/transcript.js` | 未使用的库文件 |
| 删除 `daemon.js` | 旧版 MCP 服务器，已被 `daemon.py` 替代 |
| 更新 `install.js` | MCP 配置从 `daemon.js` → `daemon.py` |
| 修复 `daemon.py` | 移除硬编码路径，修复 regex 警告 |

### V3.1 (2026-05-24)

**API 优化：减少 60% token 消耗**

| 改动 | 文件 | 说明 |
|------|------|------|
| API 统一配置 | `api_config.js` | 统一 API 调用模块，自动跟随 Claude Code 模型 |
| 合并 API 调用 | `inject.js` | 技能+记忆选择合并为 1 次 API 调用 |
| 条件任务分解 | `inject.js` | 仅短任务（≤50字符）触发分解，减少无效调用 |
| Worker API 统一 | `extract_worker.js` | 使用共享 api_config.js，自动跟随模型 |
...
| API 统一配置 | `api_config.js` | 统一 API 调用模块，自动跟随 Claude Code 模型 |
| 合并 API 调用 | `inject.js` | 技能+记忆选择合并为 1 次 API 调用（减少 60% token） |
| 条件任务分解 | `inject.js` | 仅短任务（≤50字符）触发分解，减少无效调用 |
| Worker API 统一 | `extract_worker.js` | 使用共享 api_config.js，自动跟随模型 |
