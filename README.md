...
```
settings.json Hooks（直调 launcher.exe，绕过 VBS 中间层）
  ├─ SessionStart      → launcher.exe inject.js --session-start
  ├─ UserPromptSubmit  → launcher.exe inject.js
  └─ SessionEnd        → launcher.exe consolidate.js
...
> **⚠️ 重要：hooks 必须直调 launcher.exe，不要走 VBS**
> `settings.json` 的 hooks 如果写成 `wscript.exe //B run_inject.vbs`，即使 VBS 内部调用了 launcher.exe，`WScript.Shell.Run` 这个中间层本身仍可能引起窗口闪烁。
>
> **正确的 hook 配置（直调 launcher.exe）：**
> ```json
> { "type": "command", "command": "\"C:/path/to/wiz/launcher.exe\" inject.js", "async": true }
> ```
